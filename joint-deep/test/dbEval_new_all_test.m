function [rs] = dbEval_new_all_test(FramesEval, choice, gndSel, Cnum)
...
for id=ids
  switch id
    ...
    case 15,   alg = def('ACF', 0, id);
    case 16,   alg = def('CCF', 0, id);
    case 17,   alg = def('Checkerboards+', 0, id);
    case 18,   alg = def('CompACT-Deep', 0, id);
    case 19,   alg = def('DeepParts', 0, id);
    case 20,   alg = def('FPDW', 0, id);
    case 21,   alg = def('HOG', 0, id);
    case 22,   alg = def('Katamari', 0, id);
    case 23,   alg = def('LDCF', 0, id);
    case 24,   alg = def('SA-FastRCNN', 0, id);
    case 25,   alg = def('SCF+AlexNet', 0, id);
    case 26,   alg = def('SpatialPooling+', 0, id);
    case 27,   alg = def('TA-CNN', 0, id);
    case 28,   alg = def('VJ', 0, id);
    case 29,   alg = def('JointDeep', 0, id);
    
    case 61,   alg = def('JointDeep-Caltech', 0, id);
    case 62,   alg = def('JointDeep-INRIA', 0, id);
    case 63,   alg = def('JointDeep-KITTI', 0, id);
    case 64,   alg = def('JointDeep-Caltech+INRIA+KITTI', 0, id);
    case 65,   alg = def('JointDeep-Caltech+INRIA', 0, id);
    case 66,   alg = def('JointDeep-Caltech+KITTI', 0, id);
    case 67,   alg = def('JointDeep-INRIA+KITTI', 0, id);
        
    case 68,   alg = def('Torch-JointDeep-Caltech', 0, id);
    case 69,   alg = def('Torch-JointDeep-INRIA', 0, id);
    case 70,   alg = def('Torch-JointDeep-KITTI', 0, id);
